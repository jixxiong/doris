...
public:
    ...
    CuckooHashCodeGenerator() = default;
    CuckooHashCodeGenerator(CuckooHashFunc hash_fn) : _hash_fn(hash_fn) {}

    [[nodiscard]] uint32_t hash(uint32_t key, uint32_t scalar = 0) const {
        uint32_t result;
        _hash_fn(reinterpret_cast<const char*>(&key), sizeof(key), scalar * CUCKOO_HASH_SEED,
                 static_cast<void*>(&result));
        return result;
    }

    [[nodiscard]] uint32_t hash(Slice key, uint32_t scalar = 0) const {
        uint32_t result;
        _hash_fn(key.get_data(), key.get_size(), scalar * CUCKOO_HASH_SEED,
                 static_cast<void*>(&result));
        return result;
    }

    [[nodiscard]] CuckooFinger get_finger(Slice key) const {
        CuckooFinger finger = hash(key);
        return UNLIKELY(finger == EMPTY_SLOT_FINGER) ? DEFAULT_NOT_EMPTY_SLOT_FINGER : finger;
    }

private:
    CuckooHashFunc _hash_fn;
};
...
public:
    ...
    // Max number of table
    static constexpr uint32_t MAX_NUM_LARYERS = 64;

    // Max memory useage of cuckoo table, here is 1GB
    static constexpr size_t MAXIMUM_BYTES = 1ull * 1024 * 1024 * 1024;

    CuckooTable() = default;
    ~CuckooTable() = default;
    CuckooTable(const CuckooTable&) = default;
    CuckooTable(CuckooTable&&) = default;
    CuckooTable& operator=(CuckooTable const&) = default;
    CuckooTable& operator=(CuckooTable&&) = default;

    Status init(CuckooTableOptions opts) {
        _len_table = opts.layer_size;
        _num_layers = opts.num_layers;
        ...
        _generator = CuckooHashCodeGenerator(murmur_hash3_x86_32);

        if (_len_table) {
            _table = std::vector<Slot>(_num_layers * _len_table);
            if (!_read_only) {
                _visited = std::vector<bool>(_num_layers * _len_table);
            }
        }
        return Status::OK();
    }

    Status init(CuckooTable const& other, uint32_t layer_size) {
    ...
    Status insert(Slice key, CuckooValue value) {
        ...
        auto cur_finger = _generator.get_finger(key);
    ...
    [[nodiscard]] FindCoroutine find_coro(Slice key) const {
        auto finger = _generator.get_finger(key);
        for (uint32_t layer = 0; layer < _num_layers; ++layer) {
            auto hash_code = _generator.hash(finger, layer);
    ...
    [[nodiscard]] uint32_t capacity() const { return _len_table * _num_layers; }
    ...
    Status resize(uint32_t len_table) {
        if (!_is_length_ok(len_table)) {
            auto msg = fmt::format(
                    ...
                    _min_length(), _max_length(), _num_layers, len_table, _occupied);
            LOG(INFO) << msg;
            return Status::InvalidArgument(msg);
        }
        CuckooTable new_table;
        RETURN_IF_ERROR(new_table.init(*this, len_table));
        for (auto [finger, value] : _table) {
            if (UNLIKELY(finger == EMPTY_SLOT_FINGER)) {
                continue;
            }
            if (new_table._insert(finger, value).is<ErrorCode::NOT_FOUND>()) {
                LOG(INFO) << fmt::format(
    ...
    [[nodiscard]] uint32_t num_layers() const { return _num_layers; }

    [[nodiscard]] size_t size() const { return _num_layers * _len_table * sizeof(Slot); }
...
private:
    ...
    Status _insert(CuckooFinger finger, CuckooValue value) {
        ...
        for (uint32_t cur_node_id = 0; cur_node_id < search_tree.size(); ++cur_node_id) {
            ...
            for (uint32_t nxt_layer = 0; nxt_layer < _num_layers; ++nxt_layer) {
                auto hash_code = _generator.hash(cur_finger, nxt_layer);
    ...
    Status _check_arguments() {
        if (_num_layers > MAX_NUM_LARYERS) {
            return Status::InvalidArgument(
                    "number of layers({}) shuld be no more than MAX_NUM_LARYERS({})", _num_layers,
                    MAX_NUM_LARYERS);
        }
        ...
        if (!_is_length_ok(_len_table)) {
            auto msg = fmt::format(
                    ...
                    "length should be within the interval [{}, {}], but now "
                    "num_layers: {}, length: {}, occupied: {}, ",
                    _min_length(), _max_length(), _num_layers, _len_table, _occupied);
            LOG(INFO) << msg;
            return Status::InvalidArgument(msg);
        }
        return Status::OK();
    ...
    [[nodiscard]] uint32_t _min_length() const {
        return ceil(_occupied / _max_density / _num_layers);
    }

    [[nodiscard]] uint32_t _max_length() const {
        return floor(MAXIMUM_BYTES / (_num_layers * sizeof(Slot)));
    }
    ...
    CuckooHashCodeGenerator _generator;
};
