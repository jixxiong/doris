    ...
    if (_tablet->enable_unique_key_merge_on_write()) {
        ...
        if (segments.size() > 1) {
            ...
            if (config::enable_vmiter) {
                ...
                RETURN_IF_ERROR(_tablet->calc_delete_bitmap_between_segments(_cur_rowset, segments, _delete_bitmap, _cur_max_version));
            } else {
                LOG(INFO) << "calculate delete bitmap between segments without VMergeIterator";
                RETURN_IF_ERROR(_tablet->calc_delete_bitmap_between_segments_without_VMIterator(_cur_rowset, segments, nullptr,
